...
@interface NSString (DiffMatchPatchPrivate)
...
@interface NSMutableString (DiffMatchPatchPrivate)

- (void)unescapeForEncodeUriCompatability;

@end

@interface NSMutableArray (DiffMatchPatchPrivate)
		...
		while (YES) {
			if ([[vMap objectAtIndex:d] containsObject:[self diffFootprintX:x - 1 y:y]]) {
				...
				if (lastOp == BDiffDelete) {
					[(id)[[path objectAtIndex:0] text] insertString:[text1 substringWithRange:NSMakeRange(x, 1)] atIndex:0];
				} else {
				...
				if (lastOp == BDiffInsert) {
					[(id)[[path objectAtIndex:0] text] insertString:[text2 substringWithRange:NSMakeRange(y, 1)] atIndex:0];
				} else {
	...
	for (NSInteger d = [vMap count] - 2; d >= 0; d--) {
		while (YES) {
			if ([[vMap objectAtIndex:d] containsObject:[self diffFootprintX:x - 1 y:y]]) {
				x--;
				if (lastOp == BDiffDelete) {
					[(id)[[path lastObject] text] appendString:[text1 substringWithRange:NSMakeRange([text1 length] - x - 1, 1)]];
				} else {
				...
				y--;
				if (lastOp == BDiffInsert) {
					[(id)[[path lastObject] text] appendString:[text2 substringWithRange:NSMakeRange([text2 length] - y - 1, 1)]];
				} else {
		...
		while ([bigpatch.diffs count] != 0) {
			...
			while ([bigpatch.diffs count] != 0 && patch.length1 < patchSize - Patch_Margin) {
				...
				if (diffType == BDiffInsert) {
					...
					if ([diffText isEqualToString:[[bigpatch.diffs objectAtIndex:0] text]]) {
						...
						[(id)[[bigpatch.diffs objectAtIndex:0] text] replaceCharactersInRange:NSMakeRange(0, [diffText length]) withString:@""];
					}
				}
			}
			...
			if ([postcontext length] != 0) {
				...
				if ([patch.diffs count] != 0 && [[patch.diffs lastObject] operation] == BDiffEqual) {
					[(id)[[patch.diffs lastObject] text] appendString:postcontext];
				} else {
...
@implementation NSString (DiffMatchPatchPrivate)
...
@implementation NSMutableString (DiffMatchPatchPrivate)
...
@implementation NSMutableArray (DiffMatchPatchPrivate)
...
NSString *DiffTypeAttributeName = @"DiffTypeAttributeName";